...
func TestExtractIntegration_UpstreamUnavailable(t *testing.T) {
	var calls atomic.Int32
	gemini := newGeminiStub(t, func(w http.ResponseWriter, _ *http.Request) {
		calls.Add(1)
		w.WriteHeader(http.StatusServiceUnavailable)
	})
	srv, jwks, _ := setupServerWithPublisher(t, nil, ai.New(gemini.URL, "gemini-test", "server-key"))
	token := jwks.mint(t, uuid.New().String(), time.Hour)
	...
	require.JSONEq(t, `{"error":"ai_upstream_unavailable"}`, string(body))
